# src/rhosocial/activerecord/backend/impl/mysql/examples/named_migrations/run_chain.py
"""
Multi-step migration chain example — two migrations with dependency (MySQL).
...
Usage:
    python -m rhosocial.activerecord.backend.impl.mysql.examples.named_migrations.run_chain
"""
...
BASE = "rhosocial.activerecord.backend.impl.mysql.examples.named_migrations.migrations"


def main():
    print("=" * 60)
    print("Named Migration Demo — Dependency Chain (MySQL)")
    print("=" * 60)
    ...
    store_path = Path(tempfile.gettempdir()) / "mig_mysql_chain.json"
    ...
    if store_path.exists():
        store_path.unlink()
    print("\n=== MySQL chain migration demo completed ===")
