...
# Supported status types for CLI
...
def handle_status_sync(args, backend: MySQLBackend, provider: Any):
...
async def handle_status_async(args, backend: AsyncMySQLBackend, provider: Any):
...
def _resolve_mysql_config(args) -> MySQLConnectionConfig:
    """Resolve MySQL connection config with priority: explicit params > named connection.
