...
class MySQLPartitionMixin:
    ...
    def get_partition_lifecycle_provider(self) -> Any:
        return MySQLPartitionLifecycleProvider(self)
...
class MySQLPartitionLifecycleProvider:
    """Construct MySQL partition lifecycle expressions."""
    ...
    _SUPPORT_METHODS = {
        ...
        PartitionOperation.REORGANIZE: "supports_reorganize_partition",
        PartitionOperation.EXCHANGE: "supports_exchange_partition",
        PartitionOperation.REMOVE_PARTITIONING: "supports_remove_partitioning",
        PartitionOperation.COALESCE: "supports_coalesce_partition",
        PartitionOperation.ANALYZE: "supports_analyze_partition",
        PartitionOperation.CHECK: "supports_check_partition",
        PartitionOperation.OPTIMIZE: "supports_optimize_partition",
        PartitionOperation.REBUILD: "supports_rebuild_partition",
        PartitionOperation.REPAIR: "supports_repair_partition",
    }
    ...
    def _require_type(self, request: PartitionRequest, request_type: type) -> None:
        if not isinstance(request, request_type):
            raise PartitionLifecycleContractError(
                f"MySQL partition provider received {type(request).__name__} for "
    ...
    def build(self, request: PartitionRequest) -> BaseExpression:
        ...
        if request.operation is PartitionOperation.ADD:
            ...
            if not isinstance(request.definition, MySQLPartitionDefinition):
                raise TypeError(
                    "MySQL add_partition requires MySQLPartitionDefinition, "
        ...
        if request.operation is PartitionOperation.DROP:
            ...
            return MySQLDropPartitionExpression(
                self.dialect,
                request.table,
        ...
        if request.operation is PartitionOperation.TRUNCATE:
            ...
            return MySQLTruncatePartitionExpression(
                self.dialect,
                request.table,
                [request.partition_name],
            )
        if request.operation is PartitionOperation.REORGANIZE:
            self._require_type(request, ReorganizePartitionRequest)
            if any(
                not isinstance(definition, MySQLPartitionDefinition)
                for definition in request.definitions
            ):
                raise TypeError("MySQL reorganize_partition requires MySQL definitions")
        ...
        if request.operation is PartitionOperation.EXCHANGE:
            ...
            return MySQLExchangePartitionExpression(
                self.dialect,
                request.table,
                request.partition_name,
                request.exchange_table,
                with_validation=request.with_validation,
            )
        if request.operation is PartitionOperation.REMOVE_PARTITIONING:
            self._require_type(request, RemovePartitioningRequest)
            return MySQLRemovePartitioningExpression(self.dialect, request.table)
        if request.operation is PartitionOperation.COALESCE:
            self._require_type(request, CoalescePartitionsRequest)
            return MySQLCoalescePartitionExpression(
                self.dialect,
                request.table,
                request.count,
            )
        if request.operation in {
            PartitionOperation.ANALYZE,
            PartitionOperation.CHECK,
            PartitionOperation.OPTIMIZE,
            PartitionOperation.REBUILD,
            PartitionOperation.REPAIR,
        }:
            self._require_type(request, MaintenancePartitionRequest)
            expression_types = {
                PartitionOperation.ANALYZE: MySQLAnalyzePartitionExpression,
                PartitionOperation.CHECK: MySQLCheckPartitionExpression,
                PartitionOperation.OPTIMIZE: MySQLOptimizePartitionExpression,
                PartitionOperation.REBUILD: MySQLRebuildPartitionExpression,
                PartitionOperation.REPAIR: MySQLRepairPartitionExpression,
            }
            return expression_types[request.operation](
                self.dialect,
                request.table,
                [request.partition_name],
            )
