# src/rhosocial/activerecord/backend/impl/mysql/introspection.py
"""
MySQL database introspection implementation.

This module provides MySQL-specific introspection using information_schema
system tables.

Key information_schema tables used:
- SCHEMATA: Database information
- TABLES: Table metadata
- COLUMNS: Column metadata
- STATISTICS: Index information
- KEY_COLUMN_USAGE + REFERENTIAL_CONSTRAINTS: Foreign key information
- VIEWS: View definitions
"""

from typing import Optional, List, Dict

from rhosocial.activerecord.backend.introspection.mixins import IntrospectionMixin
...
class MySQLIntrospectionMixin(IntrospectionMixin):
    """
    ...
    def _execute_introspection_query(self, sql: str, params: tuple = ()) -> List[Dict]:
        ...
        This method is intentionally named differently from _execute_query to avoid
        signature conflict with StorageBackend._execute_query which takes (cursor, sql, params).
        """
    ...
    def _query_database_info(self) -> DatabaseInfo:
        """Query MySQL database information."""
        # Get version
        rows = self._execute_introspection_query("SELECT VERSION()")
        ...
        rows = self._execute_introspection_query("""
    ...
    def _query_tables(self, schema: Optional[str] = None,
        ...
        """
        rows = self._execute_introspection_query(sql, (db_name,))
        ...
        for row in rows:
            ...
            table_type = table_type_map.get(row['TABLE_TYPE'], TableType.BASE_TABLE)

            # Skip system tables
            if not include_system and table_type == TableType.SYSTEM_TABLE:
                continue
    ...
    def _query_table_info(self, table_name: str,
    ...
    def _query_columns(self, table_name: str,
        ...
        """
        rows = self._execute_introspection_query(sql, (db_name, table_name))
    ...
    def _query_indexes(self, table_name: str,
    ...
    def _query_foreign_keys(self, table_name: str,
    ...
    def _query_views(self, schema: Optional[str],
    ...
    def _query_view_info(self, view_name: str,
    ...
    def _query_triggers(self, table_name: Optional[str] = None,
        ...
        rows = self._execute_introspection_query(sql, tuple(params))
