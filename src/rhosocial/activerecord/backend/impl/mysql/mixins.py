...
class MySQLIntrospectionMixin:
    ...
    def format_trigger_list_query(self, expr: "TriggerListExpression") -> Tuple[str, tuple]:
        ...
        Args:
            expr: Trigger list expression with optional table_name and schema.
...
class MySQLPartitionMixin:
    ...
    def supports_partitioned_table_creation(self) -> bool:
        """Whether CREATE TABLE can create partitioned tables."""
        return True
    ...
    def supports_subpartitioning(self) -> bool:
        """Whether table subpartitioning is supported through current expressions."""
        return False
    ...
    def supports_add_partition(self) -> bool:
        """Whether ALTER TABLE ... ADD PARTITION is supported."""
        return True

    def supports_drop_partition(self) -> bool:
        """Whether ALTER TABLE ... DROP PARTITION is supported."""
        return True

    def supports_truncate_partition(self) -> bool:
        """Whether ALTER TABLE ... TRUNCATE PARTITION is supported."""
        return True

    def supports_reorganize_partition(self) -> bool:
        """Whether ALTER TABLE ... REORGANIZE PARTITION is supported."""
        return True

    def supports_attach_partition(self) -> bool:
        """Whether ATTACH PARTITION syntax is supported."""
        return False

    def supports_detach_partition(self) -> bool:
        """Whether DETACH PARTITION syntax is supported."""
        return False
    ...
    def supports_partition_definition_options(self) -> bool:
        """Whether partition definitions support extra options."""
        return True
...
class MySQLJsonDualityViewMixin:
    ...
    Provides SQL generation for CREATE/DROP JSON RELATIONAL DUALITY VIEW
    statements using JSON_DUALITY_OBJECT with WITH(...) annotations.
    """

    def supports_json_duality_view(self) -> bool:
        """JSON Duality Views are supported since MySQL 9.7."""
        return self.version >= (9, 7, 0)

    def supports_json_duality_view_dml(self) -> bool:
        """DML on JSON Duality Views is supported since MySQL 9.7."""
        return self.version >= (9, 7, 0)
    ...
    def format_create_json_duality_view_statement(self, expr) -> Tuple[str, tuple]:
        ...
        select_clause = self._format_duality_object_select(expr.root_spec)
    ...
    def _format_duality_object_body(self, spec) -> str:
        ...
        for nested in spec.nested:
            nested_subquery = self._format_nested_duality(nested)
    ...
    def _format_nested_duality(self, nested) -> str:
        """Format a nested JSON_ARRAYAGG(JSON_DUALITY_OBJECT(...)) subquery."""
        sub_spec = nested.subquery
        obj_body = self._format_duality_object_body(sub_spec)
...
class MySQLOptimizerHintMixin:
    ...
    Provides SQL generation for per-statement optimizer hints
    using the /*+ ... */ syntax, including SET_VAR hints.
    """

    def supports_optimizer_hint(self) -> bool:
        """Optimizer hints supported since MySQL 5.7."""
        return self.version >= (5, 7, 0)

    def supports_hypergraph_optimizer(self) -> bool:
        """Hypergraph optimizer available in MySQL 9.7+ Community Edition."""
        return self.version >= (9, 7, 0)
