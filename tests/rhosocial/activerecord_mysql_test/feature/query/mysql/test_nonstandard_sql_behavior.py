...
async def test_aggregate_with_order_by_no_group_by(async_order_fixtures):
    ...
    order = AsyncOrder(user_id=user.id, order_number="MYSQL-AGG-001", total_amount=Decimal("100.00"))
    await order.save()
...
async def test_exists_with_retained_order_by(async_order_fixtures):
    ...
    order = AsyncOrder(user_id=user.id, order_number="MYSQL-EXISTS-001", total_amount=Decimal("50.00"))
    await order.save()
    ...
    # MySQL allows this non-standard behavior
    exists = await async_query.where(AsyncOrder.c.order_number == "MYSQL-EXISTS-001").exists()
