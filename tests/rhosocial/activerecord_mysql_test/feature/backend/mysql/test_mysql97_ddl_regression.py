# tests/rhosocial/activerecord_mysql_test/feature/backend/mysql/test_mysql97_ddl_regression.py
"""
...
class TestPrimaryKeyEquivalentDDL:
    ...
    def test_unique_not_null_as_pk_equivalent(self, mysql_backend):
        ...
        backend.execute(
            "INSERT INTO pk_equiv_test (code, name) VALUES (%s, %s)",
            ("A001", "Test")
        )
    ...
    def test_create_table_with_invisible_column(self, mysql_backend):
        ...
        backend.execute(
            "INSERT INTO invis_col_test (name) VALUES (%s)", ("hello",)
        )
...
class TestMySql97FunctionRegression:
    ...
    def test_default_function(self, mysql_backend):
        ...
        backend.execute(
            "UPDATE default_fn_test SET status = 'inactive' WHERE id = 1", ()
        )
        backend.execute(
            "UPDATE default_fn_test SET status = DEFAULT(status) WHERE id = 1", ()
        )
    ...
    def test_date_functions(self, mysql_backend):
        ...
        result = backend.execute(
            "SELECT TIMEDIFF('12:00:00', '10:30:00') AS diff", ()
        )
        ...
        result = backend.execute(
            "SELECT ADDDATE('2026-05-30', INTERVAL 1 DAY) AS d", ()
        )
    ...
    def test_find_in_set(self, mysql_backend):
        ...
        result = backend.execute(
            "SELECT FIND_IN_SET('b', 'a,b,c,d') AS pos", ()
        )
        assert result.data[0]["pos"] == 2

        result = backend.execute(
            "SELECT FIND_IN_SET('x', 'a,b,c') AS pos", ()
        )
    ...
    def test_in_with_null(self, mysql_backend):
        ...
        """, (), options=ddl_opts)
        backend.execute(
            "INSERT INTO in_null_test VALUES (1, 10), (2, NULL), (3, 30)", ()
        )

        result = backend.execute(
            "SELECT id FROM in_null_test WHERE val IN (10, NULL) ORDER BY id", ()
        )
        assert len(result.data) == 1
    ...
    def test_decimal_comparison(self, mysql_backend):
        ...
        """, (), options=ddl_opts)
        backend.execute(
            "INSERT INTO decimal_test VALUES (1, 19.99), (2, 20.00), (3, 20.01)", ()
        )

        result = backend.execute(
            "SELECT id FROM decimal_test WHERE price > 19.999 ORDER BY id", ()
        )
