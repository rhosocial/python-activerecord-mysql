# tests/rhosocial/activerecord_mysql_test/feature/backend/ddl/test_ddl_source_collection.py
...
SQL_TYPE = MySQLIntType(unsigned=True)
FALLBACK_SQL_TYPE = MySQLLongTextType()
...
COLLATION = CollationAttribute(name="utf8mb4_0900_ai_ci")
...
TABLE_PARTITION = MySQLPartitionByRange(
    DIALECT,
...
SINGLE_SQL_TYPE = MySQLIntType(unsigned=True, zerofill=True)
...
def test_backend_types_and_column_options_are_collected_without_dialect_selection():
    ...
    assert isinstance(sql_type.data_type, MySQLIntType)
    ...
    assert isinstance(option, MySQLColumnOptions)
    assert option.character_set == "utf8mb4"
    assert option.column_format is MySQLColumnFormat.FIXED
    assert option.storage is MySQLColumnStorage.MEMORY
...
def test_table_declarations_preserve_identity_order_and_composite_primary_key():
    ...
    assert isinstance(TABLE_OPTIONS[0], MySQLCreateTableOptions)
...
def test_collected_table_options_render_with_mysql_dialect():
