# tests/rhosocial/activerecord_mysql_test/feature/backend/adapters/test_enum_adapter_backend.py
"""
...
class TestMySQLEnumAdapterBackend:
    ...
    def test_enum_with_sql_execution(self, mysql_backend):
        ...
        mysql_backend.execute("INSERT INTO test_enum_table (status, priority) VALUES (%s, %s)", ("published", 3))

        # Query and verify
        result = mysql_backend.execute("SELECT status, priority FROM test_enum_table WHERE id = %s", (1,))
    ...
    def test_enum_null_handling_with_database(self, mysql_backend):
        ...
        # Insert NULL
        mysql_backend.execute("INSERT INTO test_enum_null (status) VALUES (NULL)")

        # Query and verify NULL
        result = mysql_backend.execute("SELECT status FROM test_enum_null WHERE id = %s", (1,))
    ...
    def test_native_mysql_enum_type(self, mysql_backend):
        ...
        db_value = adapter.to_database(Status.PUBLISHED, str)
        mysql_backend.execute("INSERT INTO test_native_enum (status) VALUES (%s)", (db_value,))

        # Query and convert back
        result = mysql_backend.execute("SELECT status FROM test_native_enum WHERE id = %s", (1,))
...
class TestAsyncMySQLEnumAdapterBackend:
    ...
    async def test_async_enum_null_handling_with_database(self, async_mysql_backend):
        ...
        # Insert NULL
        await async_mysql_backend.execute("INSERT INTO test_async_enum_null (status) VALUES (NULL)")

        # Query and verify NULL
        result = await async_mysql_backend.execute("SELECT status FROM test_async_enum_null WHERE id = %s", (1,))
