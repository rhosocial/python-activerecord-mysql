...
"""
Concrete implementation of IBasicConnectionProvider for MySQL backend.
...
class BasicConnectionProvider(IBasicConnectionProvider):
    """
    MySQL backend implementation for basic connection pool context tests.
    """
    ...
    def setup_sync_pool_and_model(self, scenario_name: str) -> Tuple[BackendPool, Type[ActiveRecord]]:
        ...
        SyncTestUser.configure(config, MySQLBackend)
    ...
    async def setup_async_pool_and_model(self, scenario_name: str) -> Tuple[AsyncBackendPool, Type[AsyncActiveRecord]]:
        ...
        # Configure model
        await AsyncTestUser.configure(config, AsyncMySQLBackend)
