...
"""
Concrete implementation of IQueryConnectionProvider for MySQL backend.
...
class QueryConnectionProvider(IQueryConnectionProvider):
    """
    MySQL backend implementation for query connection pool context tests.
    """
    ...
    def setup_sync_pool_and_model(self, scenario_name: str) -> Tuple[BackendPool, Type[ActiveRecord]]:
        ...
        SyncQueryTestUser.configure(config, MySQLBackend)
    ...
    async def setup_async_pool_and_model(self, scenario_name: str) -> Tuple[AsyncBackendPool, Type[AsyncActiveRecord]]:
        ...
        # Configure model
        await AsyncQueryTestUser.configure(config, AsyncMySQLBackend)
