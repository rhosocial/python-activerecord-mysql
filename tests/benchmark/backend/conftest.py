"""MySQL direct backend benchmark fixtures."""
...
SCENARIO_PARAMS = list(get_enabled_scenarios().keys()) or [
    pytest.param("default", marks=pytest.mark.skip(reason="No MySQL benchmark scenarios found"))
]
...
def pytest_addoption(parser):
    try:
        parser.addoption(
            ...
            help="Data size for MySQL backend benchmark scenarios.",
        )
    except ValueError:
        pass
...
def mysql_backend_sync_context(request, benchmark_size):
    ...
    try:
        yield BackendBenchmarkContext(
            ...
            backend_namespace="rhosocial.activerecord.backend.impl.mysql",
            backend_name="mysql",
        )
    finally:
        backend.disconnect()
...
def _seed_sync(backend, payloads):
    ...
    for payload in payloads:
        ...
        if result.affected_rows != 1 or result.last_insert_id is None:
            raise AssertionError("failed to seed sync MySQL backend benchmark row")
...
async def _seed_async(backend, payloads):
    ...
    for payload in payloads:
        ...
        if result.affected_rows != 1 or result.last_insert_id is None:
            raise AssertionError("failed to seed async MySQL backend benchmark row")
        record_ids.append(result.last_insert_id)
    return record_ids
