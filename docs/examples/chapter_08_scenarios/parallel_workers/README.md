# 并行 Worker 场景实验（MySQL 版）

本目录包含《场景实战 — 并行 Worker》章节的完整可运行实验代码，所有示例均使用真实的 `rhosocial-activerecord` ORM 操作 MySQL 数据库，模型体系为：
...
| 文件 | 主题 | 类型 |
| ---- | ---- | ---- |
| `config_loader.py` | 从 YAML / 环境变量 / 默认值加载 MySQL 连接配置 | 工具 |
| `setup_db.py` | 初始化数据库和测试数据 | 工具 |
| `models.py` | 共享 ActiveRecord 模型定义（同步 + 异步） | 公共 |
| `exp1_basic_multiprocess.py` | 多进程正确用法 + 串行/并行耗时对比 | ✅ 正确示范 |
| `exp2_mysql_async_advantage.py` | MySQL 异步真正优势 + 并发写入能力 | ✅ 正确示范 |
| `exp3_deadlock_wrong.py` | 行锁顺序冲突导致 MySQL 死锁 | ❌ 反面教材 |
| `exp4_partition_correct.py` | 数据分区 + 原子领取 + 死锁重试 | ✅ 正确示范 |
| `exp5_multithread_warning.py` | 多线程共享 MySQL 连接的危险 | ❌ 反面教材 |

## MySQL vs SQLite 关键差异

| 特性 | SQLite | MySQL |
| ---- | ------ | ----- |
| 异步后端 | `aiosqlite`（线程池模拟，有额外开销） | `mysql-connector-python` 原生 async（网络 I/O） |
| 并发写入 | 需要 WAL 模式才能并发 | 默认行级锁，开箱即用并发 |
| 死锁检测 | 无自动检测，超时返回 locked | InnoDB 自动检测，回滚其中一方 |
| 线程安全 | `check_same_thread=True` 默认保护 | 无内置保护，官方明确说明不支持 |
| 连接方式 | 文件路径 | 网络 TCP 连接（host:port） |
| 同进程 asyncio.gather | 不支持（线程池单通道） | 不支持（单连接模型） |

## 依赖说明

本实验依赖 `mysql-connector-python`，版本要求因 Python 版本而异：

| Python 版本 | mysql-connector-python 版本  | 说明                           |
| ----------- | ---------------------------- | ------------------------------ |
| 3.8         | `==9.0.0`                    | 锁定版本（3.8 支持的最高版本） |
| 3.9+        | `>=9.1.0`（如 9.6.0）        | 使用更高版本                   |
...
```bash
# 在 python-activerecord-mysql 项目根目录
source .venv3.8-examples/bin/activate
# 或
source .venv3.14t-examples/bin/activate
```
...
实验脚本自动读取 `tests/config/mysql_scenarios.yaml`（如存在），优先使用 `mysql_80` 场景。
也可通过环境变量覆盖：
...
```bash
python setup_db.py --scenario mysql_84
```
...
python setup_db.py && python exp2_mysql_async_advantage.py
...
MySQL 特别说明：父进程 `configure()` 建立 TCP 连接后 `fork`，子进程继承文件描述符，多进程共享同一 TCP socket，比 SQLite 共享文件句柄更危险。

### exp2：MySQL 异步特点与多进程并发写入

说明 `mysql-connector-python` 异步后端（单连接模型）的特点：

- **Part 1**：同进程内，同步串行 vs 异步顺序，说明单连接不能 `asyncio.gather` 并发
- **Part 2**：多进程并发写入，MySQL 行级锁无需 WAL 配置即支持并发

预期输出（参考）：

```text
Part 1：同进程内同步串行 vs 异步顺序（单连接限制）
  同步串行：20 篇，耗时 0.XXXs
  异步并发：20 篇，耗时 0.XXXs
  异步/同步比：X.XXx（单连接下两者相近，并发优势在多进程间体现）
```

> **注意**：`mysql-connector-python` 异步后端是单连接模型，同进程内协程不能并发（不同于连接池方案）。多进程间每进程独立连接，才是并发的体现。

### exp3：反面教材 — MySQL 死锁

故意展示不一致的行锁顺序导致 MySQL 死锁（errno 1213）的场景。

MySQL InnoDB 会**自动检测死锁**并回滚代价较小的事务，另一方继续执行。
被回滚的 Worker 收到 `OperationalError`，若不处理则工作丢失。

预期输出（参考）：

```text
=== 错误模式：不一致的锁顺序（MySQL 死锁演示）===
  Worker 0 (✅ 成功)：...
  Worker 1 (💀 死锁回滚)：...
  → 死锁（MySQL errno 1213）：...
```

### exp4：正确的并行方案（含 MySQL 特有方案 C）

提供五种生产可用的并行策略：

- **方案 A 同步/异步**：按 `user_id` 分区，遍历 `HasMany` 关联统计评论数
- **方案 B 同步/异步**：事务内原子领取（`with/async with Post.transaction()`）
- **方案 C 同步**：原子领取 + 死锁自动重试（**MySQL 生产环境推荐**）
...
MySQL 无 `check_same_thread` 保护，但官方同样明确不支持多线程并发共享连接。
...
A: 检查 `tests/config/mysql_scenarios.yaml` 中的连接参数，或通过环境变量 `MYSQL_HOST` / `MYSQL_PORT` / `MYSQL_USER` / `MYSQL_PASSWORD` 配置本地 MySQL 服务器。
...
A: 取决于网络延迟和并发数。MySQL 服务器在本地时差异较小；远程服务器网络延迟较大时，异步加速效果更明显。与 SQLite（异步通常更慢）完全相反。

**Q: 为什么 exp4 的 view_count 验证用 `> 0` 而不是 `== 1`？**

A: `view_count = 1 + len(approved_comments)`，大多数文章有 1-2 条已审核评论，实际值为 2 或 3，因此验证 `> 0` 即可确认"被处理过一次"。
...
Sync serial: 0.210s, async sequential: 0.212s

Part 2: Multiprocess concurrent writes (MySQL row-level locking)
Sync multiprocess (4 processes): 1.025s
Async multiprocess (4 processes): 1.096s
```
...
⚠️ Found 2 Workers rolled back due to deadlock by MySQL!
All posts have view_count=1 (correct values after rollback)
```
...
Sync: 1.006s, Async: 1.108s ✓ No duplicates

Solution B: Atomic Claiming
Sync: 1.428s, Async: 1.199s ✓ No duplicates

Solution C: Atomic + Deadlock Retry (Production Recommended)
Sync: 1.629s ✓ No duplicates
```
...
⚠️ Found 2 errors (proves multithreaded connection sharing is unsafe)
...
这是因为 Windows 默认使用 `ProactorEventLoop`，而 `mysql-connector-python` 的异步后端需要 `SelectorEventLoop`。
