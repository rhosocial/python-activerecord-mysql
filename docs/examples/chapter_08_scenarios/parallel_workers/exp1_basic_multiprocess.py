...
using real MySQL ActiveRecord ORM.
...
MySQL-specific notes:
  - MySQL connections are network connections, each configure() establishes a new TCP connection
...
def worker_sync(post_ids: list) -> int:
    ...
    1. Call configure() within the process, establishing independent MySQL network connection
...
async def async_worker_main(post_ids: list) -> int:
    ...
    would cause "read() called while another coroutine is waiting" error.
    Concurrency is achieved across multiple processes: each process holds an independent
    MySQL connection. True single-process concurrent queries example in exp2_mysql_async_advantage.py.
    """
    config = load_config()
...
Important differences between MySQL and SQLite:
  - configure() establishes real TCP network connections, inheriting connection file
    descriptors after fork is more dangerous
  - Async backend (mysql-connector-python native async) is based on real TCP network I/O,
    unlike SQLite's aiosqlite thread simulation
  - Experiment C's async version typically shows more speedup than SQLite async
    version (see exp2)
""")
