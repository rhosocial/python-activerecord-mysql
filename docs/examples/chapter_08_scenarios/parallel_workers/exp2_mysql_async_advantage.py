# exp2_mysql_async_advantage.py - MySQL Async Features and Multiprocess Concurrent Writes
# docs/examples/chapter_12_scenarios/parallel_workers/exp2_mysql_async_advantage.py
"""
Experiment objective:
    Explain the single-connection limitation of mysql-connector-python async backend
    (AsyncMySQLBackend), and how MySQL row-level locking enables multiprocess concurrent
...
Experiment has two parts:
  ...
  Part 1 — Single-connection limitation (sync vs async within same process)
    ...
    mysql-connector-python async backend is single-connection, concurrent access raises RuntimeError.
  ...
  Part 2 — Multiprocess concurrent writes (no WAL mode configuration needed)
    MySQL InnoDB defaults to row-level locking, multiple processes writing to different
    rows won't block each other.
    Contrast with SQLite: default file locking serializes all writes, requires WAL mode
    for concurrency.
    Conclusion: MySQL supports true concurrent writes out of the box.

How to run:
    python setup_db.py   # Initialize database first
    python exp2_mysql_async_advantage.py
"""
...
async def run_async_concurrent(post_ids: list) -> float:
    ...
    cause "read() called while another coroutine is waiting" error).
...
# ─────────────────────────────────────────────────────────────────────────────
# Part 2: Multiprocess concurrent writes (row-level locking, no WAL config needed)
# ─────────────────────────────────────────────────────────────────────────────
...
def main() -> None:
    ...
    print(f"\n{'=' * 60}")
    print("Part 2: Multiprocess concurrent writes (MySQL row-level locking, no WAL config needed)")
    print("=" * 60)
...
Conclusion:
  ...
  Concurrency is achieved across multiple processes: each process holds independent MySQL TCP connection,
  enabling true concurrency between processes.

  MySQL multiprocess concurrent writes require no extra configuration (no WAL, no journal_mode),
  InnoDB row-level locking by default allows multiple processes to write different rows without
  blocking each other.

  Async (AsyncMySQLBackend) advantages:
