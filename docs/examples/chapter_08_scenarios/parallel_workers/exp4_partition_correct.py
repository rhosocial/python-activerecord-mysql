# exp4_partition_correct.py - Data Partitioning + Atomic Claiming Correct Implementation (MySQL Version)
# docs/examples/chapter_12_scenarios/parallel_workers/exp4_partition_correct.py
"""
Experiment objective:
    Demonstrate two correct multiprocess parallel processing strategies using real MySQL ActiveRecord ORM.
    ...
    Solution B: Atomic Claiming (query + update within transaction)
        ...
        MySQL InnoDB row-level locking makes this pattern more efficient than SQLite (no file-level lock contention).

    Solution C: Atomic Claiming + Deadlock Retry
        Built on Solution B, catch MySQL deadlock exception (errno 1213) and automatically retry.
...
def _is_deadlock(exc: Exception) -> bool:
    """Check if exception is MySQL deadlock (errno 1213)"""
    msg = str(exc)
...
def claim_posts_sync(batch: int = CLAIM_BATCH) -> list:
    ...
    MySQL InnoDB row-level locking ensures only one Worker can successfully claim the same batch.
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# Solution C: Atomic Claiming + Deadlock Retry (MySQL Production Recommended)
# ═══════════════════════════════════════════════════════════════════════════════


def claim_posts_with_retry(batch: int = CLAIM_BATCH, max_retry: int = MAX_RETRY) -> list:
    """Atomic claiming with MySQL deadlock exception (errno 1213) catch and retry."""
...
def worker_atomic_retry(worker_id: int) -> int:
    """Sync Worker (with deadlock retry): MySQL production recommended solution"""
    config = load_config()
    User.configure(config, MySQLBackend)
    Post.__backend__ = User.backend()
    Comment.__backend__ = User.backend()
...
def main() -> None:
    ...
    print("\n=== Solution C: Atomic Claiming + Deadlock Retry (MySQL Production Recommended, Sync) ===")
    reset_all_posts()
    t0 = time.perf_counter()
    with multiprocessing.Pool(processes=NUM_WORKERS) as pool:
    ...
    print("""
MySQL-specific optimization notes:
  - Solution A (Data Partitioning): Each Worker operates on different rows, MySQL row-level
    locking has no contention, highest efficiency
  - Solution B (Atomic Claiming): InnoDB row-level locking precisely locks limit rows,
    MySQL performs better than SQLite
  ...
  - Compared to SQLite WAL mode, MySQL supports concurrent writes without extra configuration
""")
