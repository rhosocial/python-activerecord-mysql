...
Experiment objective:
    ...
    MySQL detects deadlock and forcibly rolls back one of the transactions (raises
    OperationalError/DatabaseError, errno=1213).
...
Deadlock scenario:
    ...
    waits for row 1, MySQL InnoDB automatically detects the deadlock and rolls back
    one transaction, allowing the other to proceed.

Note:
    - This is an anti-pattern demonstrating unsafe lock ordering
    - MySQL deadlock detection is automatic, the rolled-back process receives OperationalError
...
def worker_wrong_order(args: tuple) -> dict:
    ...
    try:
        ...
        if "1213" in err_str or "deadlock" in err_str.lower():
            result["deadlock"] = True
            result["error"] = f"Deadlock (MySQL errno 1213): {err_str[:80]}"
        else:
...
def main() -> None:
    print("=== Anti-pattern: Inconsistent Lock Order (MySQL Deadlock Demo) ===\n")
    ...
    if deadlock_count > 0:
        print(f"\n⚠️  Found {deadlock_count} Workers rolled back due to deadlock by MySQL!")
...
Conclusion (anti-pattern):
  - MySQL InnoDB automatically detects deadlocks (deadlock detection algorithm, enabled by default)
  - When deadlock occurs, MySQL chooses the lower-cost transaction to roll back, allowing the other to proceed
  - Rolled-back Worker receives OperationalError (errno=1213)
