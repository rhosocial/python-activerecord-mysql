# setup_db.py - MySQL Parallel Worker Experiment Database Initialization Script
...
Usage:
    ...
    python setup_db.py --scenario mysql_84   # Specify YAML scenario
...
def init_sync(scenario: str = None) -> None:
    ...
    # Create tables (execute statement by statement, MySQL doesn't support batch multi-statement execute)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Initialize parallel worker experiment database (MySQL)")
    parser.add_argument("--async", dest="use_async", action="store_true", help="Use asynchronous initialization mode")
    parser.add_argument("--scenario", default=None, help="Specify YAML scenario name (e.g., mysql_80)")
    ...
    print("  python setup_db.py && python exp2_mysql_async_advantage.py")
